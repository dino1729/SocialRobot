"""Entrypoint for the internet-connected voice assistant with wake word detection and tool support (Online LiteLLM version)."""
...
from llm.litellm import LiteLLMClientWithTools
...
def main(
    ...
    """
    print("-> Initializing internet-connected voice assistant with wake word detection (Online LiteLLM version)...")
    ...
    system_prompt = f"""You are a world-class knowledgeable AI voice assistant, Orion, hosted on a Jetson Orin Nano Super. Your mission is to assist users with any questions or tasks they have on a wide range of topics. Use your knowledge, skills, and resources to provide accurate, relevant, and helpful responses. 

As a voice assistant, provide clear and complete answers that are easy to understand when spoken aloud. For simple questions, keep responses concise (2-3 sentences). For complex topics, detailed questions, or when users ask for news/information, provide thorough explanations with sufficient detail to be truly helpful - aim for 4-6 sentences or more when needed to fully address the question. When presenting search results or news, summarize the key points clearly. Always prioritize clarity and completeness over brevity. 

Be polite, friendly, and respectful in your interactions. Don't include any emojis, asterisks, or formatting symbols in your responses since they don't work well in speech.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Internet-connected voice assistant with wake word detection and tool support (Online LiteLLM version)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Use default settings (auto-detects compute type, uses Kokoro TTS)
  python main_wakeword_internetconnected_online.py
  
  # Use Piper TTS instead of Kokoro
  python main_wakeword_internetconnected_online.py --tts-engine piper
  
  # Use Piper TTS with GPU acceleration
  python main_wakeword_internetconnected_online.py --tts-engine piper --tts-gpu
  
  # Use Kokoro with different voice and speed
  python main_wakeword_internetconnected_online.py --tts-engine kokoro --tts-voice af_sarah --tts-speed 1.2
  
  # Use int8 compute type (for Jetson Orin Nano)
  python main_wakeword_internetconnected_online.py --compute-type int8
  
  # Use float16 compute type (for desktop GPUs like RTX 5090)
  python main_wakeword_internetconnected_online.py --compute-type float16
  
  # Custom wake word threshold
  python main_wakeword_internetconnected_online.py --wakeword-threshold 0.6
  
  # Disable memory monitoring
  python main_wakeword_internetconnected_online.py --no-memory-monitor
  
  # Change memory monitor interval
  python main_wakeword_internetconnected_online.py --monitor-interval 30
  
  # Combine multiple options
  python main_wakeword_internetconnected_online.py --tts-engine piper --tts-gpu --compute-type int8 --wakeword-threshold 0.55
        """,
    )
